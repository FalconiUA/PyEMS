...
def test_single_target_written_clamped_to_envelope():
    ...
    assert a.resolve([req(target_w=95000.0, max_w=200000.0)], now=0.0) == 80000.0
...
def test_constraint_only_plus_lower_priority_target():
    ...
    assert a.resolve([cap, plan], now=0.0) == 40000.0
...
def test_disjoint_ranges_higher_priority_wins_and_logs_once(caplog):
    ...
    with caplog.at_level(logging.WARNING):
        v1 = a.resolve([hi, lo], now=0.0)
        warns = [r for r in caplog.records if "rejected" in r.message]
        v2 = a.resolve([hi, lo], now=1.0)  # steady state -> no repeat warning
...
def test_no_target_holds_last_after_first_cycle():
    a = arbiter()
    a.resolve([req("x", target_w=40000.0)], now=0.0)          # establishes last
    # next cycle only a pure constraint that permits a wide range -> hold 40k.
    held = a.resolve([req("x", max_w=90000.0)], now=1.0)
    assert held == 40000.0
...
def test_deadband_suppresses_micro_move():
    ...
    a.resolve([req("x", target_w=40000.0)], now=0.0)
    # +1 kW request is within deadband -> stays at 40k.
    assert a.resolve([req("x", target_w=41000.0)], now=1.0) == 40000.0
...
def test_priority_zero_bypasses_deadband():
    ...
    assert a.resolve([safe], now=1.0) == 41000.0
...
def test_ramp_limits_the_step():
    ...
    a.resolve([req("x", target_w=100000.0)], now=0.0)   # first cycle lands at 100k
...
def test_priority_zero_bypasses_ramp():
    ...
    assert a.resolve([safe], now=1.0) == 50000.0
...
def test_determinism_regardless_of_post_order():
    ...
    assert a1.resolve([r1, r2], now=0.0) == a2.resolve([r2, r1], now=0.0)
